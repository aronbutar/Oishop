...
public class ProductNameService {
    ...
    public static void initDatabase() {
        Nitrite database = Nitrite.builder()
                .filePath(getPathToFile("product-name1.db").toFile())
                .openOrCreate("test", "test");


        productNameRepository = database.getRepository(ProductName.class);
    }
    public static void addName(String  name, String category,String publish, int price) throws NameAlreadyExistsException {
        checkNameDoesNotAlreadyExist(name);
        productNameRepository.insert(new ProductName(name,category,publish,price));
